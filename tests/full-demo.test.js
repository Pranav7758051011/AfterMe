...
async function runFullGoldenDemo() {
  ...
  // STEP 7 & 8: Grounded Conversational Retrieval
  console.log('\n--- Step 7 & 8: Ask AfterMe Conversational Retrieval ---');
  console.log('User Question: "Where did I leave my charger?"');
  const askRes = await request('/api/ask', 'POST', {
    question: 'Where did I leave my charger?',
    user_id: 'demo_user_001'
  });
  console.log('AfterMe AI Answer:');
  console.log('  >>', askRes.answer);
  console.log('  Has Verified Match:', askRes.has_match);
  console.log('  Cited Records:', askRes.relevant_memories?.map(m => `${m.object} @ ${m.location}`));

  // STEP 9: Rejection of Non-existent item
  console.log('\n--- Step 9: Grounded Retrieval Test (Unknown Item Rejection) ---');
  console.log('User Question: "Where are my sunglasses?"');
  const unknownRes = await request('/api/ask', 'POST', {
    question: 'Where are my sunglasses?',
    user_id: 'demo_user_001'
  });
  console.log('AfterMe AI Answer (Context Grounded Rejection):');
  console.log('  >>', unknownRes.answer);
  console.log('  Has Match:', unknownRes.has_match);

  // STEP 10: Multi-Tenant Isolation
  console.log('\n--- Step 10: Multi-Tenant User Isolation Test ---');
  const userBMemories = await request('/api/memories', 'GET', null, 'user_bob_999');
  ...
  // STEP 11: Mark Retrieved
  console.log('\n--- Step 11: User Retrieves Charger ---');
  const updateRes = await request(`/api/memories/${createRes.memory.id}/status`, 'PATCH', {
    status: 'retrieved',
    user_id: 'demo_user_001'
  });
  console.log('Status updated in Firestore:', updateRes.memory.status);

  // STEP 12: Contextual Linked Memory
  console.log('\n--- Step 12: Contextual Linked Memories Demo ---');
  await request('/api/memories', 'POST', {
    text: 'I have a visa appointment tomorrow at 10 AM.',
    current_location: 'Embassy Office',
    user_id: 'demo_user_001'
  });
  const contextAsk = await request('/api/ask', 'POST', {
